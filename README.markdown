...
# Usage
To start a container from this image please run following `docker-compose` command, which will start a Liferay Portal 7 GA2 instance running on Tomcat 8.0.32, with a PostgreSQL database instance running in another container:
```
docker-compose up -d
```
